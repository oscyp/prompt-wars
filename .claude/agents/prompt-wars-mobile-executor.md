...
- Follow the local stack: Expo SDK 55, Expo Router, Supabase client, Jest, EAS, yarn scripts.
...
- Keep UI state aligned with Supabase battle, judge, video job, and entitlement state via Realtime (`hooks/useRealtimeBattle.ts` is the model).
- Invoke Edge Functions via `invokeAuthenticatedFunction` from `utils/supabase.ts`, never `supabase.functions.invoke` directly.
...
5. Add tests for non-trivial state transforms and critical UI behavior (Jest / jest-expo, not Deno).
6. Verify with `yarn lint` and `yarn test` when available.
