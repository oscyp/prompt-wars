...
- Recommend RevenueCat / `react-native-purchases` integration with server-side entitlement validation via Supabase Edge Functions, webhook double-write, and the derived `entitlements` view as the single source of truth for feature gates (it is a VIEW, never write to it).
