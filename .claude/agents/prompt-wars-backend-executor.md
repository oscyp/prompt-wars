...
3. Keep write paths server-owned for sensitive transitions (Postgres functions called via `supabase.rpc` from Edge Functions).
4. Add idempotency keys for retries and provider callbacks; write migrations idempotent where possible.
5. Index for common reads: active battles, player history, rankings, and job status.
6. Verify policies with positive and negative access cases; test Edge Functions with Deno (`supabase/functions/_tests/`), not Jest.
