...
Deno.serve(async (req) => {
  ...
  // Service-role only (server-owned battle resolution)
  ...
  try {
    ...
    // Insert judge run
    const { error: judgeRunError } = await supabase
      .from('judge_runs')
      .insert({
        battle_id,
    ...
    try {
      await generateTier0Reveal(supabase, battle_id, battle, p1Prompt, p2Prompt, winnerId, judgeResult.is_draw, scorePayload);
    } catch (tier0Error) {
      console.error('Tier 0 reveal generation failed (non-blocking):', tier0Error);
...
async function generateTier0Reveal(
  ...
  const tier0Payload = {
    ...
    metadata: {
      winnerCharacter: winnerId === battle.player_one_id ? battle.player_one_character?.name : playerTwoCharacter?.name,
      loserCharacter: winnerId === battle.player_one_id ? playerTwoCharacter?.name : battle.player_one_character?.name,
      winnerArchetype: winnerId === battle.player_one_id ? battle.player_one_character?.archetype : playerTwoCharacter?.archetype,
      winnerColor: winnerId === battle.player_one_id ? battle.player_one_character?.signature_color : playerTwoCharacter?.signature_color,
