...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Check eligibility
    const { data: canAppeal, error: eligibilityError } = await supabase.rpc(
      'can_appeal',
      {
        p_profile_id: userId,
    ...
    if (!canAppeal) {
      return errorResponse(
        'Appeal not eligible (daily cap or battle constraints)',
        403,
      );
    }

    // Submit appeal
    const { data: appealId, error: appealError } = await supabase.rpc(
      'submit_appeal',
      {
        p_battle_id: battle_id,
        p_profile_id: userId,
    ...
    if (appealError) {
      ...
      return errorResponse(
        appealError.message || 'Failed to submit appeal',
        400,
      );
    }
