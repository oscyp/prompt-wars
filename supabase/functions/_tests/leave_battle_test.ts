...
Deno.test(
  'leaveOutcomeFor: only a matched ranked human match is a forfeit',
  () => {
    const cases: {
      mode: string;
...
Deno.test(
  'buildLeaveScorePayload: no monetization data reaches scoring',
  () => {
...
Deno.test(
  'buildLeaveScorePayload: single format carries no series fields',
  () => {
    const payload = buildLeaveScorePayload({
      leaverId: LEAVER,
      format: 'single',
...
Deno.test('buildLeaveScorePayload: rating_gated only when a gate fired', () => {
  const ungated = buildLeaveScorePayload({
    leaverId: LEAVER,
    format: 'single',
  });
...
Deno.test(
  'leaveIdempotencyKey: stable, and independent of round and clock',
  () => {
    ...
    assertEquals(
      first,
      'leave_battle_battle-1_11111111-1111-1111-1111-111111111111',
    );
