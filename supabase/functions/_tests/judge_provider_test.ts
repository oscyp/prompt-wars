...
class ExplodingJudge implements AiJudgeProvider {
  ...
  judge(_req: JudgeRequest): Promise<JudgeResponse> {
    ...
    return Promise.reject(
      new JudgeProviderError('timeout', 'simulated outage'),
    );
  }
}
...
Deno.test(
  'FallbackJudgeProvider - provider failure degrades to mock',
  async () => {
...
Deno.test(
  'FallbackJudgeProvider - fallback output still passes schema validation',
  async () => {
    const provider = new FallbackJudgeProvider(
      new ExplodingJudge(),
      new MockJudgeProvider(),
...
Deno.test(
  'FallbackJudgeProvider - healthy provider is used, mock is not',
  async () => {
...
Deno.test(
  'createJudgeProvider - JUDGE_PROVIDER=xai is wrapped in a fallback',
  () => {
...
Deno.test(
  'createJudgeProvider - defaults to mock, unknown value falls back',
  () => {
    const previous = Deno.env.get('JUDGE_PROVIDER');
    try {
...
Deno.test(
  'XAIJudgeProvider - missing API key fails fast with a typed error',
  async () => {
