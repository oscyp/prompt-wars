...
Deno.test('look - drops a field staged back to its saved value', () => {
  ...
  assert(plan.ok);
  assertEquals(plan.changed.map((c) => c.field), ['era']);
});

Deno.test('look - a no-op batch changes nothing', () => {
  // Would otherwise bump appearance_version and falsely mark the portrait stale.
  const plan = planLookBatch(character, { vibe: 'heroic', palette_key: 'ember' });
  assert(plan.ok);
  assertEquals(plan.changed.length, 0);
  assertEquals(Object.keys(plan.update).length, 0);
});

Deno.test('look - validates everything before reporting any of it applicable', () => {
  const plan = planLookBatch(character, { era: 'cyberpunk', vibe: 'confused' });
  assertEquals(plan.ok, false);
  if (!plan.ok) assertEquals(plan.field, 'vibe');
});

Deno.test('look - clearing the prompt is a real change, not an absent field', () => {
...
Deno.test('look - rejects an over-long prompt', () => {
  assert('reason' in validateLookField('portrait_prompt_raw', 'x'.repeat(PROMPT_MAX + 1)));
  assert('value' in validateLookField('portrait_prompt_raw', 'x'.repeat(PROMPT_MAX)));
});
...
Deno.test('look - the item is never null', () => {
  ...
  assert('value' in validateLookField('signature_item_id', '11111111-1111-1111-1111-111111111112'));
});

Deno.test('look - traits and art style share one audit kind, palette and item keep theirs', () => {
  const kindOf = (f: string) => LOOK_FIELDS.find((d) => d.field === f)?.logKind;
...
Deno.test('look - every log kind is one the character_edits CHECK allows', () => {
...
Deno.test('random - shuffles all five traits into valid values', () => {
  for (let i = 0; i < 40; i++) {
    ...
    assertEquals(Object.keys(t).sort(), [
      'era', 'expression', 'palette_key', 'silhouette', 'vibe',
    ]);
  }
});
