...
function createMockSupabase(fx: Fixtures): any {
  const one = (table: string, filters: Record<string, unknown>) => {
    switch (table) {
      ...
      case 'prompt_templates':
        return {
          data: fx.templates?.[String(filters.id)] ?? null,
          error: null,
        };
      default:
  ...
  const many = (table: string) => {
    if (table === 'battle_prompts')
      return { data: fx.prompts ?? [], error: null };
  ...
  const from = (table: string) => {
    ...
    const api = {
      ...
      // deno-lint-ignore no-explicit-any
      then: (res: any, rej: any) => Promise.resolve(many(table)).then(res, rej),
    };
    return api;
  };
...
Deno.test(
  'composeRevealPayload — human battle populates flat compat fields + signs winner portrait',
  async () => {
    const fx: Fixtures = {
      battle: {
        id: 'battle-1',
    ...
    assertEquals(
      payload.players.player_two.portrait.fallback_gradient.length,
      2,
    );
    assertEquals(
    ...
    assertEquals(
      payload.judge.why,
      'Player one was clearer and more specific.',
    );
...
Deno.test(
  'composeRevealPayload — Bo3 round reads per-round outcome; bot opponent gets gradient fallback',
  async () => {
    const fx: Fixtures = {
      battle: {
        id: 'battle-2',
