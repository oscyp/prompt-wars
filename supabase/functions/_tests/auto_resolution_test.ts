...
Deno.test({
  ...
  async fn() {
    ...
    // Create battle
    const { data: battleId, error: battleError } = await supabase.rpc(
      'create_battle',
      {
    ...
    const { data: prompt1Id, error: lock1Error } = await supabase.rpc(
      'lock_prompt',
      {
        p_battle_id: battleId,
    ...
    const { data: prompt2Id, error: lock2Error } = await supabase.rpc(
      'lock_prompt',
      {
        p_battle_id: battleId,
    ...
    assertEquals(
      battle.status,
      'resolving',
      'Battle should be in resolving status after both prompts locked',
    );
