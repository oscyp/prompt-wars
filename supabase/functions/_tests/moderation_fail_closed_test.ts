...
Deno.test('blocklist matches whole words, not substrings', async () => {
  await withEnv({ OPENAI_API_KEY: null, PERSPECTIVE_API_KEY: null }, async () => {
...
Deno.test('no provider configured: blocklist only, still approves clean text', async () => {
  await withEnv({ OPENAI_API_KEY: null, PERSPECTIVE_API_KEY: null }, async () => {
...
Deno.test('configured provider that fails is held for review, not approved', async () => {
